...
void	ft_powerset(int *set, int size, int target, int index, int *power_set, int size_powerset)
{
	...
	// Je teste en ajoutant la valeur a la combinaison
	power_set[size_powerset] = set[index];
	ft_powerset(set, size, target, index + 1, power_set, size_powerset + 1);
	// Je teste  sans ajouter la valeur a la combinaison
...
int	main(int ac, char **av)
{
	...
	if (ac < 3)
		return (1);
	target = atoi(av[1]); // target = la valeur a atteindre
	size = ac - 2;
	set = malloc(size * sizeof(int)); //un tableau pour les valeurs a tester
	if (!set)
		return (1);
	power_set = malloc(size * sizeof(int)); // un tableau pour les solutions
	if (!power_set)
		return (1);
	i = -1;
	while (++i < size) // stocker les valeurs a tester dans power set
		set[i] = atoi(av[i + 2]); 
	ft_powerset(set, size, target, 0, power_set, 0); // chercher les combinaisons
	free(set);
	free(power_set);
	return (0);
}
